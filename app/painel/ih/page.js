...
export default function PainelTV() {
  ...
  async function carregar() {
    const { data: painel } = await supabase.from("vw_painel_tv_ih").select("*");
    setUnidadesRanking(painel || []);
    const { data: acess } = await supabase.from("vw_painel_acessorios_ih").select("*").order("total_vendido", { ascending: false });
    setAcessorios(acess || []);
  }
    ...
    <div data-theme="claro" className="h-screen overflow-hidden bg-canvas text-ink px-[2vw] py-[2vh] font-body flex flex-col">
      <div className="flex justify-between items-center mb-[1.5vh] shrink-0">
        <div>
          ...
          <h1 className="font-display text-[3.4vh] font-semibold leading-tight flex items-center gap-[1.2vh]">
            {periodo.rotulo}
            <span className="text-[1.6vh] px-[1.2vh] py-[0.4vh] rounded-full bg-teal text-white font-medium tracking-wide">IH</span>
          </h1>
        </div>
