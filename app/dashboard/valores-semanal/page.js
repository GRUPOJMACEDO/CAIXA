...
import { Wallet, CheckCircle2, Clock, Percent, Hash, Lock, Info, CalendarRange, FileSpreadsheet, Printer } from "lucide-react";
...
import { hojeBrasil, listaSemanasRecentes } from "../../../lib/fusoHorario";
...
function ConteudoDashboard() {
  ...
  async function carregar() {
    setCarregando(true);
    const dados = filtrarPorMarca(await buscarValoresPorPeriodo(supabase, semana.inicio, diaSeguinte(semana.fim), linhaFiltro), marcasFiltro);
  ...
  useEffect(() => {
    carregar();
  }, [linhaFiltro, marcasFiltro, semanaSelecionada]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  async function exportarExcel() {
    ...
    XLSX.writeFile(livro, `valores-semanal-W${semana.numero}-${semana.inicio}.xlsx`);
  }
    ...
    <div className="max-w-6xl">
      <div className="mb-6 flex items-start justify-between gap-4">
        <div className="min-w-0">
          ...
          <h1 className="font-display text-2xl font-semibold text-ink">Valores (Semanal) — W{semana.numero} · {formatarDataBR(semana.inicio)} a {formatarDataBR(semana.fim)}</h1>
          <p className="text-sm text-muted mt-1">Ranking de todas as unidades do grupo, semana de domingo a sábado.</p>
        </div>
        <div className="flex items-center gap-2 print:hidden shrink-0">
          <div className="relative">
            <CalendarRange size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-muted pointer-events-none z-10" />
            <select
              className="field-input pl-8 py-2 w-56"
      ...
      <div className="grid grid-cols-5 gap-3 mb-6">
        ...
        <div className="card overflow-hidden">
          ...
          <div className="p-4">
            ...
            <p className="text-[10px] text-muted mt-1 leading-snug">Só das OS movimentadas nessa semana — para o total em aberto de tudo, veja Contas a Receber.</p>
          </div>
        </div>
        <div className="card overflow-hidden">
      ...
      <div className="card overflow-hidden">
        <table className="w-full text-sm">
          <thead>
            <tr className="text-xs uppercase tracking-wider text-muted border-b border-line">
              ...
              <td className="p-3 text-right">
                <span className="inline-flex items-center gap-1" title="Só considera o orçamento definido e o valor pago nessa semana — não é o saldo total em aberto dessas OS. Para o total, veja Contas a Receber.">
          ...
          <tbody>
            ...
            {!carregando && ordenadas.length === 0 && <tr><td className="p-4 text-muted" colSpan={5}>Nenhum lançamento nessa semana.</td></tr>}
            {ordenadas.map((l, i) => (
