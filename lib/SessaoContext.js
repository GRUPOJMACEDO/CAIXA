...
export function SessaoProvider({ children }) {
  ...
  // Logout automático depois de 2h sem nenhuma interação (mouse, teclado,
  // toque ou scroll) — evita que abas esquecidas abertas fiquem consumindo
  // banco (polling, realtime) sem ninguém usando de fato.
  useEffect(() => {
    if (!usuario || typeof window === "undefined") return;
    const LIMITE_INATIVIDADE_MS = 2 * 60 * 60 * 1000; // 2 horas
    const VERIFICA_A_CADA_MS = 60 * 1000; // checa a cada 1 min
