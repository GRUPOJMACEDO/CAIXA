...
// ADM e Auditoria enxergam todas as unidades (uso geral / leitura), mas não mexem em Configurações
export function podeVerTodasUnidades(cargo) {
  return [CARGOS.ADM, CARGOS.ADMINISTRADOR, CARGOS.DIRETOR, CARGOS.AUDITORIA].includes(cargo);
}
...
// Cadastro de técnicos (dentro da tela de Unidades) — igual Categorias/Modelos
export function podeConfigTecnicos(cargo) {
  return [CARGOS.SUPERVISAO, CARGOS.GERENCIA, CARGOS.ADMINISTRADOR, CARGOS.DIRETOR].includes(cargo);
}
...
// Auditoria de lançamentos em data retroativa: Administrador, Diretor e o
// próprio cargo Auditoria (é o motivo dele existir)
export function podeVerAuditoriaRetroativos(cargo) {
  return [CARGOS.ADMINISTRADOR, CARGOS.DIRETOR, CARGOS.AUDITORIA].includes(cargo);
}
// Exportar para Excel na Consulta — restrito a Gerência, Auditoria e
// Administrador (reduz consumo de banco/egress; evita exportações soltas
// por qualquer login).
export function podeExportarConsulta(cargo) {
  return [CARGOS.GERENCIA, CARGOS.AUDITORIA, CARGOS.ADMINISTRADOR].includes(cargo);
}
// Módulo Duplicidades — fica no menu Operação, não em Configurações.
// Supervisão e Gerência (donos da unidade), mais Administrador e Diretor.
export function podeVerDuplicidades(cargo) {
  return [CARGOS.SUPERVISAO, CARGOS.GERENCIA, CARGOS.ADMINISTRADOR, CARGOS.DIRETOR].includes(cargo);
}
export function podeRevisarDuplicidades(cargo) {
  return podeVerDuplicidades(cargo);
}
export function temAcessoConfiguracoes(cargo) {
  return (
    ...
    podeVerAuditoriaRetroativos(cargo)
  );
}
